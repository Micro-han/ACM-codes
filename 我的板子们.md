...
public:
    ...
    int pos;
    int dp[11][11][11][22];
    int dfs(int pos, bool lead, bool limit) {
        if(pos == -1) {
          // 当pos==-1时代表枚举到了最后一个数位，这时根据题目需要返回答案
            if () return 1;
            else return 0;
        }
        // 如果这个数字被枚举过了
      	if(!limit && !lead && dp[pos][state] != -1) return dp[pos][state];
        int ans = 0, up = limit ? a[pos] : 9; // 判断上界
        for(int i = 0; i <= up; i++) {
            if () {} 
            else {};
...
## 统计区间[low, high]数位中奇数个数与偶数个数相同且本身为k的倍数的数字个数
...
public:
    ...
    int pos;
    int dp[11][11][11][22];
    int dfs(int pos, bool lead, bool limit, int kk, int od, int ev, int res) {
        if(pos == -1) {
            return od == ev && res == 0;
        }
        if(!limit && dp[pos][od][ev][res] != -1 && !lead) return  dp[pos][od][ev][res];
        int ans = 0, up = limit ? a[pos] : 9;
        for(int i = 0; i <= up; i++) {
            ans += dfs(pos - 1, lead && i == 0, limit && i == a[pos], kk, od + (i % 2 == 1), ev + (!(lead && i == 0) && (i % 2 == 0)), (res * 10 + i) % kk);
        } 
        if(!limit) return dp[pos][od][ev][res] = ans;
        return ans;
    }
    int solve(int x, int k) {
        ...
        memset(dp, -1, sizeof(dp));
        return dfs(pos - 1, true, true, k, 0, 0, 0);
    }
...
## 统计区间[low, high]中数位和在[min_sum, max_sum]之间的数字个数

```c++
class Solution {
public:
    int mod = 1e9 + 7, dp[30][404];
    int dfs(string s, int n, int min_sum, int max_sum, int sum, int pos, bool lead, bool limit) {
        if(sum > max_sum) return 0;
        if(pos == n) return sum >= min_sum;
        if(!limit && !lead && dp[pos][sum] != -1) return dp[pos][sum];
        int ans = 0, up = limit ? s[pos] - '0': 9;
        for(int i = 0; i <= up; i++) 
            ans = (ans + dfs(s, n, min_sum, max_sum, sum + i, pos + 1, lead && i == 0, limit && i == s[pos] - '0')) % mod;
    ...
    int solve(string s, int min_sum, int max_sum) {
        memset(dp, -1, sizeof(dp));
        return dfs(s, s.length(), min_sum, max_sum, 0, 0, true, true);
    }
    int count(string num1, string num2, int min_sum, int max_sum) {
        int ans = (solve(num2, min_sum, max_sum) - solve(num1, min_sum, max_sum) + mod) % mod;
        int sum = 0, len = num1.length();
        for(int i = 0; i < len; i++) sum += num1[i] - '0';
